...
function colorRange(type) {
  return type === "Breakfast" ? "yellow":
    type === "Seafood" ? "blue" :
    type === "Vegetarian" ? "green" :
    type === "Carne Asada / Beef" ? "red" :
    type === "Carnitas / Pork" ? "pink" :
    type === "California" ? "orange" :
  ...
  if (burritos[i].type === "Breakfast") {
    breakfastmarkers.push(
      L.circle(burritos[i].location, {
        fillOpacity: 0.4,
        color: colorRange(burritos[i].type),
        fillColor: colorRange(burritos[i].type),
        // Adjust radius
        radius: burritos[i].fillings * burritos[i].fillings * burritos[i].fillings * burritos[i].fillings
    }).bindPopup("<h1>" + burritos[i].restuarant + "<h2>" + burritos[i].burrito_name + "</h1> <hr> <h3>fillings score: " + burritos[i].fillings + "</h3>" + burritos[i].full_address + "</h3>"));
  }
  else if   
...
L.control.layers(baseMaps, overlayMaps, {
  collapsed: true
      ...
      legend.onAdd = function (myMap) {
        ...
        grades = ["Breakfast", "Seafood", "Vegetarian", "Carne Asada / Beef", "Carnitas / Pork", "California", "Specialty", "Pollo"],
        labels = [];
      
        for (var i = 0; i < grades.length; i++) {
          div.innerHTML +=
            '<i style="background:' + colorRange(grades[i]) + '">&nbsp&nbsp&nbsp</i> ' +
            grades[i] + '<br>';
        }
        return div;
      };
